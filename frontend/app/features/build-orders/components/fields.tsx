/**
 * circuit.rocks — build-orders feature: brutalist field primitives.
 *
 * Co-located input helpers so the long neo-brutalist class string lives in one
 * place (mirrors the transfers feature's `components/fields.tsx`). Each field
 * takes a dotted `path` (e.g. `qtyPlanned`) and an `errors` map (from
 * `unwrapFieldErrors().fieldErrors` or `zod` issues); when present it renders
 * the message in red mono under the control.
 *
 * Re-created locally on purpose — features do NOT import helpers across feature
 * boundaries.
 */
...
/** A bare control-class input used inside tables/grids/sheets (no label wrapper). */
