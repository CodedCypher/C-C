/**
 * circuit.rocks — branches feature: brutalist field primitives.
 *
 * Co-located input helpers so the long neo-brutalist class string lives in one
 * place (mirrors the transfers/products feature field kits). Each field takes a
 * dotted `path` (e.g. `code`) and an `errors` map (from
 * `unwrapFieldErrors().fieldErrors`); when present it renders the message in red
 * mono under the control. These are feature-local on purpose — do NOT import
 * field helpers across features.
 */
