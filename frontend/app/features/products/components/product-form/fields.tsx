/**
 * circuit.rocks admin — Create-product form: brutalist field primitives.
 *
 * Co-located input helpers so the long neo-brutalist class string lives in one
 * place. Each field takes a dotted `path` (e.g. `variants.0.sku`) and an
 * `errors` map (from `useActionData().fieldErrors`); when present it renders the
 * message in red mono under the control.
 */
...
export function TextAreaField({
  ...
  return (
    <label className={cn("flex flex-col gap-1.5", className)} htmlFor={fieldId}>
      ...
      <textarea
        id={fieldId}
        className={cn(controlClass, "min-h-[5rem] resize-y", err && "border-soldout")}
        aria-invalid={err ? true : undefined}
        {...props}
      />
      {hint && !err ? (
...
export function CellInput({
  ...
  return <input className={cn(controlClass, "shadow-none", className)} {...props} />;
}
