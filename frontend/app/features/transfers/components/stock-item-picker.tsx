/**
 * circuit.rocks — transfers feature: stock-item picker (line builder cell).
 *
 * A search-as-you-type combobox backed by `GET /inventory/options`. Typing
 * filters the dropdown; picking a row reports the chosen `StockItemOption` up to
 * the line builder (which stores `stockItemId` + displays `uom`). Once an item
 * is chosen the input shows its name/SKU; the user can "Change" to re-search.
 *
...
import { useStockItemOptions } from "../hooks/use-stock-item-options";
import { controlClass } from "./fields";
import type { StockItemOption } from "../types/transfers.types";
...
export function StockItemPicker({
  ...
  onPick: (option: StockItemOption | null) => void;
  invalid?: boolean;
  "aria-label"?: string;
}) {
  ...
  const { data, isLoading, isError, isFetching } = useStockItemOptions(
    term,
    open,
  );
  const options = data ?? [];
  ...
  function choose(option: StockItemOption) {
    onPick(option);
    setOpen(false);
    setTerm("");
  }
