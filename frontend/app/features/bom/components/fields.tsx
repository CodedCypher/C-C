/**
 * circuit.rocks — bom feature: brutalist field primitives (local copy).
 *
 * Co-located input helpers so the long neo-brutalist class string lives in one
 * place. Features are self-contained — this is a deliberate re-declaration of
 * the same primitives the products/transfers features use, NOT an import.
 *
 * Each field takes a dotted `path` (e.g. `lines.0.quantity`) and an `errors`
 * map (from `unwrapFieldErrors().fieldErrors` or `zodFieldErrors`); when present
 * it renders the message in red mono under the control.
 */
...
/** A bare control-class select used inside tables/grids (no label wrapper). */
export function CellSelect({
  ...
  return (
    <select
      className={cn(controlClass, "cursor-pointer shadow-none", className)}
      {...props}
    >
      {children}
    </select>
